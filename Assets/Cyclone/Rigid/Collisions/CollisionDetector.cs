...
namespace Cyclone.Rigid.Collisions
{
    ...
    public static class CollisionDetector
    {
        ...
        public static void SphereAndHalfSpace(CollisionSphere sphere, CollisionPlane plane, CollisionData data)
        {
            ...
            // CHANGED
            if (plane.Direction.y == 1)
            {
                sphere.isCollide = true;
            }
        ...
        public static void StarAndSphere(CollisionStar one, CollisionSphere two, CollisionData data)
        {
            // Make sure we have contacts
            if (data.NoMoreContacts()) return;
            ...
            // We manually create the normal, because we have the
            // size to hand.
            two.levelFinished = true;
        }
        
        public static void SphereAndBullet(CollisionSphere sphere, CollisionBullet bullet, CollisionData data)
        {
            // Make sure we have contacts
            if (data.NoMoreContacts()) return;

            // Cache the sphere positions
            Vector3d positionOne = sphere.GetAxis(3);
            Vector3d positionTwo = bullet.GetAxis(3);
            ...
            // See if it is large enough.
            if (size <= 0.0f || size >= sphere.Radius + bullet.Radius)
                return;
            ...
            contact.ContactNormal = normal;
            contact.ContactPoint = positionOne + midline * 0.5;
            contact.Penetration = sphere.Radius + bullet.Radius - size;
            if (!bullet.IsCollide)
            {
            ...
            //contact.SetBodyData(sphere.Body, bullet.Body, data.Friction, data.Restitution);
        }
        ...
        public static void BarAndHalfSpace(CollisionBar box, CollisionPlane plane, CollisionData data)
        {
            // Make sure we have contacts
            if (data.NoMoreContacts()) return;

            // Check for intersection
            if (!IntersectionTests.BarAndHalfSpace(box, plane))
                return;
        ...
        public static void BoxAndBar(CollisionBox one, CollisionBar two, CollisionData data)
        {
            // Make sure we have contacts
            if (data.NoMoreContacts()) return;
            //if (!IntersectionTests.BoxAndBox(one, two)) return;
        ...
        public static void BoxAndSphere(CollisionBox box, CollisionSphere sphere, CollisionData data)
        {
            ...
            contact.SetBodyData(box.Body, sphere.Body, friction, data.Restitution);
            // CHANGED
            sphere.isCollide = true;
        }
                public static void BarAndSphere(CollisionBar box, CollisionSphere sphere, CollisionData data)
        {
            // Make sure we have contacts
            if (data.NoMoreContacts()) return;
        ...
        public static void SpringAndSphere(CollisionSpring box, CollisionSphere sphere, CollisionData data)
        {
            // Make sure we have contacts
            if (data.NoMoreContacts()) return;
            ...
            // CHANGED
            if (!box.isApplied || !sphere.isOnSpring)
            {
                sphere.isOnSpring = true;
                box.isApplied = true;
            }
            else
            {
        ...
        private static bool TryAxis(CollisionBox one, CollisionBar two, Vector3d axis, Vector3d toCentre, int index, ref double smallestPenetration, ref int smallestCase)
        {
            // Make sure we have a normalized axis, and don't check almost parallel axes
            if (axis.SqrMagnitude < 0.0001) return true;
            axis.Normalize();
        ...
        private static double PenetrationOnAxis(CollisionBox one, CollisionBar two, Vector3d axis, Vector3d toCentre)
        {
            // Project the half-size of one onto axis
            double oneProject = TransformToAxis(one, axis);
            double twoProject = TransformToAxis(two, axis);
        ...
        private static double TransformToAxis(CollisionBar box, Vector3d axis)
        {
            return
                box.HalfSize.x * Vector3d.AbsDot(axis, box.GetAxis(0)) +
                box.HalfSize.y * Vector3d.AbsDot(axis, box.GetAxis(1)) +
        ...
        private static void FillPointFaceBoxBox(CollisionBox one, CollisionBar two, Vector3d toCentre, CollisionData data, int best, double pen)
        {
            // This method is called when we know that a vertex from
            // box two is in contact with box one.
            var contact = data.GetContact();
        ...
        private static void FillPointFaceBoxBox(CollisionBar one, CollisionBox two, Vector3d toCentre, CollisionData data, int best, double pen)
        {
            // This method is called when we know that a vertex from
            // box two is in contact with box one.
            var contact = data.GetContact();
