...
namespace Cyclone.Rigid.Collisions
{
    ...
    public static class IntersectionTests
    {
        ...
        public static bool StarAndSphere(CollisionStar one, CollisionSphere two)
        {
            // Find the vector between the objects
            Vector3d midline = one.GetAxis(3) - two.GetAxis(3);
        ...
        public static bool SphereAndBullet(CollisionSphere sphere, CollisionBullet bullet)
        {
            // Find the vector between the objects
            Vector3d midline = sphere.GetAxis(3) - bullet.GetAxis(3);

            // See if it is large enough.
            return midline.SqrMagnitude < (sphere.Radius + bullet.Radius) * (sphere.Radius + bullet.Radius);
        }
        ...
        public static bool BoxAndBar(CollisionBox one, CollisionBar two)
        {
            // Find the vector between the two centres
            Vector3d toCentre = two.GetAxis(3) - one.GetAxis(3);
        ...
        public static bool BarAndHalfSpace(CollisionBar box, CollisionPlane plane)
        {
            // Work out the projected radius of the box onto the plane direction
            double projectedRadius = TransformToAxis(box, plane.Direction);
        ...
        private static bool OverlapOnAxis(CollisionBox one, CollisionBar two, Vector3d axis, Vector3d toCentre)
        {
            // Project the half-size of one onto axis
            double oneProject = TransformToAxis(one, axis);
            double twoProject = TransformToAxis(two, axis);
